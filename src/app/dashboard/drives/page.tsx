...
export default function DrivesPage() {
  ...
  const [allDrives, setAllDrives] = useState(0);
  const user = useUser();
    ...
    const fetchDrives = async () => {
      ...
      try {
        const response = await fetch(
          `/api/eligible-drives?admnno=${user?.admnno}`
        );
        if (!response.ok) {
        ...
        setAllDrives(data.totalDrives || 0);
      } catch (error) {
        setDrives([]);
      } finally {
        setLoading(false);
    ...
    <DashboardLayout user={user}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
          <motion.div {...fadeInUp}>
            <DrivesSidebar
              EligibleDrives={drives.length}
              userCGPA={user.academicInfo?.cgpa ?? 0}
              allDrives={allDrives}
            />
          </motion.div>
