        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <motion.div {...fadeInUp} className="xl:col-span-2">
            {activeSection === "upload" && (
              <ResumeUploadZone userEmail={user.email} />
            )}
            {activeSection === "library" && <ResumeLibrary user={user} />}
            {activeSection === "builder" && (
              //  <ResumeBuilder />
              <p>
                This Feature is under development. May appear in future updates.
              </p>
            )}
          </motion.div>
          <motion.div {...fadeInUp}>
            {user.resumeURL && <ResumeAnalytics resumeURL={user.resumeURL} />}
          </motion.div>
        </div>
      </div>
    </DashboardLayout>
